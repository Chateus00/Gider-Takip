  ...
  const mapping: Array<{ match: (value: string) => boolean; config: Omit<BrandLogoConfig, "fallbackSrc"> }> = [
    {
      ...
      config: { icon: siNetflix, iconColor: "#E50914", iconBackground: "#111111" },
    },
    {
      match: (value) => value === "spotify" || value.startsWith("spotify "),
      config: { icon: siSpotify, iconColor: `#${siSpotify.hex}`, iconBackground: "#191414" },
    },
    {
      match: (value) => value === "youtube" || value.startsWith("youtube "),
      config: { icon: siYoutube, iconColor: `#${siYoutube.hex}`, iconBackground: "#FFFFFF" },
    },
    {
      match: (value) => value.startsWith("icloud"),
      config: { icon: siIcloud, iconColor: `#${siIcloud.hex}`, iconBackground: "#F4F7FB" },
    },
    {
